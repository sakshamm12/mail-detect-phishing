...
export const analyzeURL = (url: string): URLAnalysisResult => {
  const issues: URLAnalysisResult['issues'] = [];
  ...
  try {
    ...
    const suspiciousPatterns = [
      { pattern: /[0-9]/, message: 'Domain contains numbers (possible typosquatting)', severity: 'medium' as const },
      { pattern: /-security|-verify|-account|-update|-confirm/i, message: 'Contains security-related keywords often used in phishing', severity: 'high' as const },
      { pattern: /[il1]/, message: 'Contains easily confused characters (i, l, 1)', severity: 'low' as const },
    ];
