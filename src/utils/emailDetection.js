...
export const analyzeEmail = (email) => {
  ...
  const suspiciousPatterns = [
    { pattern: /[0-9]/, message: 'Contains numbers which may indicate typosquatting', severity: 'medium' },
    { pattern: /[il1]/, message: 'Contains characters that can be easily confused (i, l, 1)', severity: 'low' },
    { pattern: /-/, message: 'Contains hyphens which are sometimes used in phishing emails', severity: 'low' },
    { pattern: /\.co$/, message: 'Uses .co TLD instead of common .com (potential typosquatting)', severity: 'medium' },
  ];
  ...
  if (score < 40 || highSeverityIssues > 0) {
    riskLevel = 'high';
  } else if (score < 70 || mediumSeverityIssues > 0) {
    riskLevel = 'medium';
  }
